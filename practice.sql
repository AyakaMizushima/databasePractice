...
-- 平均寿命が56歳から76歳の国をすべて抽出してください。
    SELECT * FROM countries WHERE 76 >= life_expectancy
    AND life_expectancy >= 56;

-- 問6
-- 国コードがNLB,ALB,DZAのもの市区町村をすべて抽出してください。
    SELECT * FROM cities WHERE country_code = 'NLB'
    OR country_code = 'ALB'
    OR country_code = 'DZA';
...
-- 全国の中から独立記念日が1990年より前または人口が10万人より多い国を全て抽出してください。
    SELECT * FROM countries WHERE indep_year < 1990
...
-- 「Arubaの人口は103000人です」
    SELECT name || 'の人口は' || population || 'です' as POPULATION FROM countries;
...
-- 全ての国と言語を表示してください。一つの国に複数言語があると思いますので同じ国名を言語数だけ出力してください。
    SELECT name, language FROM countries INNER JOIN country_languages ON countries.code = country_languages.country_code;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
    SELECT countries.name as 国名, cities.name as 市区町村名, country_languages.language
    FROM countries 
    INNER JOIN cities ON countries.code = cities.country_code
    INNER JOIN country_languages ON countries.code = country_languages.country_code
    ORDER BY countries.code, cities.id, country_languages.language; 
...
-- 全ての有名人の名前,国名、第一言語を出力してください。
    ...
    INNER JOIN country_languages ON celebrities.country_code = country_languages.country_code
    WHERE
        country_languages.percentage =
        (SELECT MAX(percentage)
        FROM country_languages AS sub_cl
            WHERE sub_cl.country_code = country_languages.country_code
        );

-- 問29
-- 全ての有名人の名前と国名をに出力してください。 ただしテーブル結合せずサブクエリを使用してください。
    SELECT name, (SELCT name FROM countries WHERE countries.code = celebrities.country_code) FROM celebrities;
...
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
    ...
    AND (SELECT MAX(age)
        FROM celebrities t1
        WHERE t1.country_code = celebrities.country_code
        ) >= 50 AND 
        (SELECT MIN(age)
        FROM celebrities t2
        WHERE t2.country_code = celebrities.country_code
        ) <= 30
    GROUP BY country_code;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
    SELECT '1980' as 誕生年, count(*) AS 人数 FROM celebrities 
    WHERE birth >= '1980-1-1'
    AND birth <= '1980-12-31'
    UNION ALL
    SELECT '1981' as 誕生年, count(*) AS 人数 FROM celebrities 
    WHERE birth >= '1981-1-1'
    AND birth <= '1981-12-31';

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
    SELECT name,
    ROUND((SELECT avg(age) 
    FROM celebrities
    WHERE celebrities.country_code = countries.code
    ), 4) AS 平均年齢
    FROM countries 
    WHERE countries.code IN (SELECT DISTINCT country_code FROM celebrities)
    ORDER BY 平均年齢 DESC;